...
$(function(){
	...
	$('form .role').bind('click',function(){
		$(this).css('color','#fff').css('background','#f7a63d').siblings(this).css('color','#333').css('background','#e3eaea');
	});
	...
	$('form').validate({
		...
		submitHandler: function(form){
			$('form').ajaxSubmit({
				...
				dataType: 'json',
				data: {
					url: 'http://site2.91miaopin.com/api/Welcome/register_tel',
	...
	$('#gain').bind('click',function(){
		$.ajax({
			...
			dataType: 'json',
			data: {
				url: 'http://site2.91miaopin.com/api/Commonapi/send_message',
				sign: $.md5('123456Lankebaov1.0'),
				client: 'android',
				st: myDate,
				data: JSON.stringify($('form').serializeObject())   //表单序列化，并且转换成json格式
